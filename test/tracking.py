...
from asyncspring.plugins import tracking
...
def test_kick():
    join_example_user()
    test_user_return_after_quit.succeed_if(
        "example" in client.tracking_registry.users and "#example" in client.tracking_registry.users[
            "example"].channels)
    kick_example_user()
...
def test_standard_who():
    signal("raw").send(client,
                       text=":irc.example.com 352 botnick #example ex3 example.net irc.example.com ex3 H :example 3")
